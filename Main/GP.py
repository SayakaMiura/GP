...
def run_pipeline(args):
    ...
    if not clonefinder_skipped:
        if "All" in processes or "CloneFinder" in processes:
            ...
            t_()
            Com=run_clonefinder(args.mode, args.input_file, args.target_dir)
            ComLs.append(' '.join(Com))
    ...
    if "All" in processes or "PhyloSignare" in processes:
        ...
        Com=run_phylosignare(target_file_path, args.control_file, target_dir)
        ComLs.append(' '.join(Com))
...
def move_folder(src, dst):
    try:
        shutil.move(src, dst)
      #  print(f"Folder '{src}' moved to '{dst}' successfully.")
    except Exception as e: pass
       # print(f"Error moving folder '{src}' to '{dst}': {e}")
def move_files(src, target, dst):
    try:
        FileLs=glob.glob(str(src)+'*'+target)
        for file in FileLs:
...
if __name__ == "__main__":
    ...
    target_dir = os.getcwd()# initial_args.target_dir
    aln_phylosig = initial_args.clonefinder_phylosig_input
 ...
 #       print(f"Error creating directory '{OutDir}': {e}")
    ...
    OutDir1=OutDir[:(len(OutDir)-len(OutDir.split('\\')[-1]))]
    OutF=open(OutDir1+'Command.txt','w')
    OutF.write('\n'.join(Command))
    OutF.close()
  #  print ('collect output files')
  ...
  #  print (OutDir+'\\'+DataID+'_clonevpresence.png', OutDir1+'\\'+DataID+'_CloneTree.png')
   ...
   # print (os.path.exists(OutDir1+'\\'+DataID+'snv_summary.txt'))
   # print (OutDir)    